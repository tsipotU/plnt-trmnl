...
**Last updated:** 2026-05-07 (**Wave 15 PWA shipped + Waves 14 / 16 cleared.** Same-day cascade: (1) Wave 14 deferred entirely — #7 (TRMNL template redesign), #138 (illustration pipeline), and #55 (TRMNL-X dual-resolution) all moved to v1.1 backlog; Wave 14 milestone closed. Trigger: design surfaced a brand-bridge gap that #7's scope didn't anticipate — Storybook is the canonical brand catalog for the web/PWA surface but does not extend onto the TRMNL rendering surface at all. **New tracking issue #197** opened for the broader work (LiquidJS-in-Storybook approach with TRMNL framework v3.1 CSS as a decorator; OG + X stories). Two placeholder Storybook stories (`TRMNL screens / OG`, `TRMNL screens / X`) added as catalog stand-ins. Design plan `docs/plans/2026-05-06-issue-7-trmnl-template-design.md` headed with a deferred admonition. (2) **Wave 15 #59 PWA install + offline shipped** — installable home-screen app with the design-system app icon; icon export driven by `ApothecaryStamp` as single source of truth via `prebuild` script (six manifest icons + favicon set from one component); `vite-plugin-pwa` with brand-aligned manifest, Workbox precache + SWR + Background Sync queues for water/calibration/archive mutations; HTML head with `viewport-fit=cover` + dual `prefers-color-scheme` theme-color and apple-touch-icon for iOS 17.4+ mode-aware home-screen icons; Storybook isolated from the PWA pipeline via `viteFinal`. Legacy 🪴 emoji assets removed. (3) **#40 closed as obsolete-by-incremental-delivery; Wave 16 milestone closed.** All five of #40's acceptance criteria had been addressed by Waves 6/13/17 + the design system itself. Per-section PlantDetail redesigns already scoped to Wave 18. (4) **Wave 15 milestone renamed** to "Wave 15 — PWA" (TRMNL-X dropped). Wave 15 closes once #59 is verified on-device. After that there are no functional waves left before v1.0.0; tag is gated on user satisfaction, not work remaining.)
