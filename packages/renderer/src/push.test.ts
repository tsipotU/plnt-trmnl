...
describe('pushToTrmnl', () => {
  ...
  const MARKUP = '<div>hello plant</div>';
  const EXPECTED_URL = `https://usetrmnl.com/api/custom_plugins/${PLUGIN_UUID}/push`;
  ...
  it('calls fetch with correct URL, headers, and body', async () => {
    ...
    await pushToTrmnl(MARKUP, API_KEY, PLUGIN_UUID);
    ...
    expect(body).toEqual({ merge_variables: { markup: MARKUP } });
  });

  it('returns success:true on 200', async () => {
    vi.mocked(fetch).mockResolvedValueOnce(new Response('{}', { status: 200 }));

    const result = await pushToTrmnl(MARKUP, API_KEY, PLUGIN_UUID);
