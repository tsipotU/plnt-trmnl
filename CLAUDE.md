...
- **Auth gate** (Wave 9, #136): `/health`, `/api/auth/*`, and `/api/feedback` are unauthenticated. Everything else under `/api/*` requires a valid session cookie when an admin password is set in `app_state`. Bootstrap mode (no `admin_password_hash` row yet) lets all traffic through; the operator claims the instance via `/welcome` using the setup token printed in server logs at first start. Public SPA routes that bypass `AuthGate`: `/welcome` and `/login`. Recovery: `node scripts/reset-password.js` inside the container.
- **`/setup` route** is the **TRMNL device setup**, not auth. Auth bootstrap lives at **`/welcome`**. Don't conflate.
- **Plant images** (Wave 9, #132): `plants.illustration_path` (existing column) holds a relative filename. When non-null, the SPA renders `/api/illustrations/:filename` from the `packages/api/assets/catalog-images/` static mount. Catalog entries can declare `image_path` which copies to the column on POST and on enrichment callback. Adding new catalog images = drop the file in `assets/catalog-images/` + add `image_path` to the entry in `plants.json`.
